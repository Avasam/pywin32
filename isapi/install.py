...
def _CreateDirectory(iis_dir, name, params):
    ...
    try:
        ...
        iis_dir.Delete("", name)
        log(2, "Deleted old directory '{}'".format(name))
    except pythoncom.com_error:
        pass

    newDir = iis_dir.Create(params.Type, name)
    log(2, "Creating new directory '{}' in {}...".format(name, iis_dir.Name))
...
def CreateDirectory(params, options):
    ...
    log(1, "Configured Virtual Directory: {}".format(params.Name))
    return target_dir
...
def CreateISAPIFilter(filterParams, options):
    ...
    try:
        ...
        log(2, "Deleted old filter '{}'".format(filterParams.Name))
    except pythoncom.com_error:
        pass
    ...
    log(1, "Configured Filter: {}".format(filterParams.Name))
    return newFilter
...
def DeleteISAPIFilter(filterParams, options):
    ...
    try:
        ...
        log(2, "ISAPI filter path '{}' did not exist.".format(ob_path))
        return
    try:
        assert filterParams.Name.strip("/"), "mustn't delete the root!"
        filters.Delete(_IIS_FILTER, filterParams.Name)
        log(2, "Deleted ISAPI filter '{}'".format(filterParams.Name))
    except pythoncom.com_error as details:
        rc = _GetWin32ErrorCode(details)
        if rc != winerror.ERROR_PATH_NOT_FOUND:
            raise
        log(2, "ISAPI filter '{}' did not exist.".format(filterParams.Name))
    ...
    log(1, "Deleted Filter: {}".format(filterParams.Name))
...
def _AddExtensionFile(module, def_groupid, def_desc, params, options):
    ...
    try:
        ...
        log(2, "Added extension file '{}' ({})".format(module, desc))
        ...
        log(2, "Failed to add extension file '{}': {}".format(module, details))
...
def _DeleteExtensionFileRecord(module, options):
    try:
        ...
        log(2, "Failed to remove extension file '{}': {}".format(module, details))
...
def CheckLoaderModule(dll_name):
    ...
    if not os.path.isfile(template):
        raise ConfigurationError("Template loader '{}' does not exist".format(template))
    ...
    if not same:
        log(2, "Updating {}->{}".format(template, dll_name))
        shutil.copyfile(template, dll_name)
        shutil.copystat(template, dll_name)
    else:
        log(2, "{} is up to date.".format(dll_name))
...
def RemoveDirectory(params, options):
    ...
    if directory is not None:
        ...
        try:
            ...
            log(2, "AppUnLoad() for {} failed: {}".format(params.Name, exc_val))
        # Continue trying to delete it.
        try:
            parent = GetObject(directory.Parent)
            parent.Delete(directory.Class, directory.Name)
            log(1, "Deleted Virtual Directory: {}".format(params.Name))
        except:
            exc_val = sys.exc_info()[1]
            log(1, "Failed to remove directory {}: {}".format(params.Name, exc_val))
...
def _PatchParamsModule(params, dll_name, file_must_exist=True):
    if file_must_exist:
        if not os.path.isfile(dll_name):
            raise ConfigurationError("{} does not exist".format(dll_name))
...
def InstallModule(conf_module_name, params, options, log=lambda *args: None):
    ...
    if not hasattr(sys, "frozen"):
        ...
        if not os.path.isfile(conf_module_name):
            raise ConfigurationError("{} does not exist".format(conf_module_name))
...
def HandleCommandLine(
    ...
    if not conf_module_name:
        ...
        try:
            ...
            log(
                2,
                "Couldn't determine the long name for {!r}: {}".format(
    ...
    try:
        ...
        if options.verbose > 1:
            traceback.print_exc()
        print("{}: {}".format(details.__class__.__name__, details))
    except KeyError:
