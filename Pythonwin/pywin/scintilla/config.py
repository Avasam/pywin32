...
class ConfigManager:
    def __init__(self, f):
        ...
        while line:
            ...
            if section == "keys":
                ...
                self.report_error(
                    "Unrecognised section header '{}:{}'".format(section, subsection)
                )
                line = fp.readline()
                lineno = lineno + 1
        if b_close:
    ...
    def report_error(self, msg):
        self.last_error = msg
        print("Error in {}: {}".format(self.filename, msg))

    def report_warning(self, msg):
        print("Warning in {}: {}".format(self.filename, msg))
...
def test():
    ...
    print("Loaded {} items in {:.4f} secs".format(len(map), took))
