...
def RegisterServer(
    ...
    if clsctx & pythoncom.CLSCTX_LOCAL_SERVER:
        if pythoncom.frozen:
            ...
            command = "{} /Automate".format(exeName)
        else:
            ...
            command = '{} "{}" {}'.format(exeName, pyfile, str(clsid))
...
def GetRegisteredServerOption(clsid, optionName):
    ...
    keyNameRoot = "CLSID\\{}\\{}".format(str(clsid), str(optionName))
    ...
    try:
        ...
        for f in (outfile, batfile):
            try:
                ...
                print("Failed to remove tempfile '{}': {}".format(f, exc))
